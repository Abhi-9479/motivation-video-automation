...
def verify_media_files():
    ...
    music_files = [f for f in os.listdir('motiv_music') if f.endswith('.mp3')] if os.path.exists('motiv_music') else []
    video_files = [f for f in os.listdir('motiv_temp') if f.endswith('.mp4')] if os.path.exists('motiv_temp') else []
...
def create_quote_content() -> tuple[str, str, str]:
    """Generates a new, unique two-part motivational quote."""
    print("🧠 Activating AI Motivation quote generator...")
        ...
        **YOUR REQUIRED OUTPUT FORMAT:**
        ...
        try:
            ...
            if part1 in used_quotes:
                print(f"⚠️ AI generated a duplicate quote. Retrying...")
                continue
...
def generate_extra_tags(title: str, quote_parts: str) -> list:
    ...
    TITLE: {title}
    CONTENT: {quote_parts}
    RULES: Return ONLY a comma-separated list of tags. Do not use hashtags (#). Include a mix of broad and specific tags (e.g., motivation, success mindset, life lessons, daily inspiration).
...
def generate_video_with_music(part1: str, part2: str, output_filename: str):
    ...
    try:
        ...
        background_video_folder = 'motiv_temp'
        ...
        state_file = 'motiv_temp/last_video_index.txt'
        last_index = -1
    ...
    if choice in ['1', '2']:
        ...
        if choice == '2':
            ...
            try:
                ...
                description = f"""{part1} {part2}\n\n#shorts #motivation #inspirational #quotes #success #mindset"""
                base_tags = ["motivation", "inspirational", "success", "mindset", "quotes", "shorts"]
