...
def create_quote_content() -> tuple[str, str, str]:
    """
    Generates a new, unique two-part motivational quote by first reading previously
    used quotes from the Google Sheet to avoid duplicates.
    """
    print("🧠 Activating AI quote generator...")
    
    # --- NEW: Read previously used quotes to build a "memory" ---
    try:
        print("📚 Reading previously generated quotes from Google Sheet...")
        # Reads all values from the first column (Part_1) of your sheet, skipping the header.
    ...
    # Add a retry loop to ensure we get a unique quote
    MAX_ATTEMPTS = 5
    for attempt in range(MAX_ATTEMPTS):
        print(f"🤖 Attempt {attempt + 1}/{MAX_ATTEMPTS}: Generating a new, unique motivational quote...")
        
        themes = [
            "perseverance", "learning from failure", "taking calculated risks",
            "the power of focus", "the importance of teamwork", "bold leadership",
            "creating your own opportunity", "the discipline of consistency",
            "having a strong vision", "the courage to start"
        ]
        chosen_theme = random.choice(themes)
        print(f"  Chosen Theme: {chosen_theme}")

        master_prompt = f"""
        You are an AI that generates a SINGLE motivational quote.

        **CRITICAL RULES:**
        1. Your quote MUST be about the specific theme of: **{chosen_theme}**.
        2. Do NOT generate a quote that is the same as or very similar to any quote in the "PREVIOUSLY USED QUOTES" list below.
        3. Your ENTIRE response MUST be in the format below. Do NOT add any extra text or commentary.
        4. Provide only ONE quote.
        5. The language must be simple and easy to understand.
        6. The quote must have a "hook" (Part 1) and a "reveal" (Part 2).
        ...
        **YOUR REQUIRED OUTPUT FORMAT:**
        ...
        try:
            ...
            # --- NEW: Check if the generated quote is a duplicate ---
            if part1 in used_quotes:
                print(f"⚠️ AI generated a duplicate quote ('{part1}'). Retrying...")
...
def generate_extra_tags(title: str, quote_parts: str) -> list:
    ...
    prompt = f"""
    Based on the following video title and content, generate a list of 10-15 relevant, popular, and SEO-friendly YouTube tags for a motivational Short.
    ...
    - Do not use hashtags (#).
    - Include a mix of broad and specific tags (e.g., motivation, success mindset, life lessons, daily inspiration).
    - Example output: tag one, another tag, specific tag, youtube shorts

    Your comma-separated list of tags:
    """
    # Using a higher temperature for creative brainstorming
...
def generate_video_with_music(part1: str, part2: str, output_filename: str):
    ...
    try:
        ...
        background_video_folder = 'video_temp'
        ...
        state_file = 'last_video_index.txt'
        last_index = -1
    ...
    if choice == '1' or choice == '2':
        ...
        if choice == '2':
            ...
            try:
                ...
                hashtags = "#shorts #ytshorts #motivation #Quickfeelfact"
                ...
                initial_tags = ["motivation", "shorts"]
                video_id = upload_video(
                    ...
                    description=f"{part1}\n\n{part2}",
                    tags=initial_tags,
                    privacy_status='public' # Uploads as private
                )
